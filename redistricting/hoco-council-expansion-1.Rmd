...
ethnicity), and election results for local races.
...
* Election results for Howard County as a whole and for individual
  ...
  County races, including for county executive and county council.
...
```{r download}
download.file("https://apps.planning.maryland.gov/redistricting/data/Adjusted2020Blockcsv.zip",
...
download.file("https://elections.maryland.gov/elections/2018/election_data/Howard_County_2018_General.csv", "Howard_County_2018_General.csv")

download.file("https://elections.maryland.gov/elections/2018/election_data/Howard_By_Precinct_2018_General.csv",
              "Howard_By_Precinct_2018_General.csv")

download.file("https://elections.maryland.gov/elections/2018/turnout/general/Official%20by%20Party%20and%20Precinct.csv",
              "Official_by_Party_and_Precinct.csv")
```

I check the MD5 hash values for the files, and stop if the contents are not
what are expected.

```{r md5_check}
stopifnot(md5sum("Voting_Precincts.geojson") == "5c732804496e978ff9d0bead4d49d23a")
stopifnot(md5sum("Adjusted2020Blockcsv.zip") == "35cf92e410897f8a171370e7effc8205")
...
md_pop_by_block_tb <- read_csv("Blocks.csv", show_col_types = FALSE) %>%
  rename(Precinct = VTD)
```
...
My goal is doing this redistricting exercise is to be able to draw
...
- Which races and ethnic groups should be considered in redistricting?
  ...
  latter, should we include multiracial people as a distinct group, or
...
- In drawing the actual council district lines, which factors should
  be given more weight? As but one example, should we put more weight
...
- The 2020 census block-level data is not necessarily accurate, since
  the US Census Bureau deliberately introduced noise into the data in
  an attempt to provide enhanced privacy for personal demographic
  information.
...
I create a data table `md_people_per_member_tb` using the Maryland
adjusted population per precinct, first summing over the precincts in
each county, then adding columns for the county names and the sizes of
their respective councils or boards of county commissioners, and then
dividing the county (adjusted) populations by those sizes.

```{r md_people_per_member_tb}
...
md_people_per_member_tb <- md_pop_by_county_tb %>%
...
ratio_val <- md_people_per_member_tb %>%
  filter(Name == "Howard County") %>%
  select(Pop_Per_Member) %>%
  as.numeric()

county_with_max_ratio_tb <- md_people_per_member_tb %>%
  arrange(desc(Pop_Per_Member)) %>%
  head(1)

county_with_min_ratio_tb <- md_people_per_member_tb %>%
...
```{r ggplot_people_per_member_tb}
md_people_per_member_tb %>%
  ...
  xlab("County") +
  ylab("People per Member") +
  labs(
    title = "People per Council Member or County Commissioner",
...
`r format(round(mean(md_people_per_member_tb$Pop_Per_Member), -3), big.mark = ",")`
people per member and a median of
`r format(round(median(md_people_per_member_tb$Pop_Per_Member), -3), big.mark = ",")`.
...
```{r ggplot_people_per_member_proposed}
md_people_per_member_tb %>%
...
See “[Differential Privacy and the 2020 Census][dp1]” for the official
justification for doing this, a [blog post by Jessica Hullman][dp2]
discussing some of the controversy around this practice, and [articles
on the RedistrictingOnline.org site][dp3] discussing how it might
affect redistricting efforts.
...
`r num_blocks_val` census blocks and a total (adjusted) population of
about
`r format(round(pop_val, -3), big.mark = ",")` people, so the number
of people per block is about
`r format(round(avg_pop_per_block_val, -1), big.mark = ",")` on
average.  If only a few people of a given race or ethnicity live in a
given census block, the noise may greatly distort their numbers as
...
md_pop_by_precinct_tb <- md_pop_by_block_tb %>%
  ...
  summarize(across(Adj_Population:Adj_NH18__2__Races, sum))
```
...
`r num_precincts_val` precincts is considerably less work than doing
so for
...
Bureau’s policy. In particular, the Bureau identifies Hispanic people
as a unitary ethnic group, regardless of their race, and then groups
all non-Hispanic people into five racial groups: White, Black, Asian
...
```{r groups_tb, echo = FALSE, results = "hide"}
...
persons of Hispanic origin, with remaining (non-Hispanic) population
...
years of age or older is similar but leans slighty whiter:
...
Black,
`r round(100 * groups_tb$Adj_NH18__2__Races / groups_tb$Adj_18__Pop, 0)`%
...
same time their numbers are so large (and growing) that it feels wrong
to leave them out of consideration in redistricting. I’ve therefore
chosen to combine the multiracial population with the other smaller
groups to form a ”rest of the population” group separate from the main
White, Asian, Black, and Hispanic groups.

A second question is to consider only people age 18 or older, or to
consider everyone, including children who can’t vote. I’ve chosen to
include everyone: there will be many young people eligible to vote in
the 2022 county council races who were not eligible in 2018. Also,
since redrawn district boundaries are typically in effect until the
next decennial census, by the time of the 2030 county council races
two-thirds of those under 18 in 2018 will be eligible to vote.
...
boundaries do not disadvantage one or the other of the two main
...
However this is misleading, for at least two reasons. First, the
parties may have different levels of success in terms of motivating
their registered voters to actually vote. For example, in the 2018
general election
...
My choice is to use both the county council election results and the
county executive election results. I include the county executive
results because they demonstrate the potential high level of
Republican vote share given strong candidates with some level of
cross-party appeal.

```{r cc_ex3_all_votes_val}
cc_ex3_all_votes_val <- all_votes_by_candidate_tb %>%
  filter(`Office Name` == "County Council", `Office District` != "003") %>%
  summarize(`Total Votes` = sum(`Total Votes`)) %>%
  as.numeric()

dem_cc_ex3_all_votes_val <- all_votes_by_candidate_tb %>%
  filter(`Office Name` == "County Council", `Office District` != "003", Party == "DEM") %>%
  summarize(`Total Votes` = sum(`Total Votes`)) %>%
  as.numeric()

rep_cc_ex3_all_votes_val <- all_votes_by_candidate_tb %>%
  filter(`Office Name` == "County Council", `Office District` != "003", Party == "REP") %>%
  summarize(`Total Votes` = sum(`Total Votes`)) %>%
  as.numeric()
```

What about the 2018 county council election results? If we exclude
District 3, in which there was no Republican candidate for county
council, about
`r format(round(cc_ex3_all_votes_val, -3), big.mark = ",")`
people voted for county council candidates, with Democratic candidates
winning about
`r format(round(100 * dem_cc_ex3_all_votes_val / cc_ex3_all_votes_val, 0), big.mark = ",")`%
of those votes and Republican candidates winning about
`r format(round(100 * rep_cc_ex3_all_votes_val / cc_ex3_all_votes_val, 0), big.mark = ",")`%.
...
I do this by adding up the votes for the Republican county council
candidates in the other four districts (Raj Kathuria, Lisa Kim, John
Liao, and David Yungmann) and comparing that total to the number of
votes for the Republican candidate for county executive (Allan
Kittleman) in those same four districts.

Note that in order to compare apples to apples I must use only votes
cast on election day, since the precinct-level results, and thus the
district-level results for the county executive race computed from
them, are for election day only.

(I also calculate the election night results for all Democratic county
council candidates outside District 3, because I will need that number
later.)

```{r rep_cc_vs_ce_val}
rep_cc_ex3_en_votes_val <- en_votes_by_precinct_tb %>%
...
rep_ce_ex3_en_votes_val <- en_votes_by_precinct_tb %>%
  filter(`Office Name` == "County Executive",
         District != "003",
         Party == "REP") %>%
  summarize(`Election Night Votes` = sum(`Election Night Votes`)) %>%
...
rep_cc_vs_ce_val <- rep_cc_ex3_en_votes_val / rep_ce_ex3_en_votes_val

dem_cc_ex3_en_votes_val <- en_votes_by_precinct_tb %>%
  filter(`Office Name` == "County Council",
         District != "003",
         Party == "DEM") %>%
  summarize(`Election Night Votes` = sum(`Election Night Votes`)) %>%
...
In 2018 Republican county council candidates outside District 3
received on average about
`r round(100 * rep_cc_vs_ce_val, 2)`% of the vote that the GOP
candidate for county executive did in those districts.
...
hypothetical Republican candidate in District 3, producing a second
wide table .est_en_votes_by_precinct_wide_tb`. This wide format is
also the format required as input to the AutoRedistrict software, so I
...
precinct for both the county council and the county executive races,
i.e., how many people voted for the Democratic candidate for county
council (or county executive) in a given precinct vs. how many people
voted for the Republican candidate for county council (or county
executive) in that same precinct.
...
districts fared relative to Kittleman.

I now have precinct-level election day vote shares for both county
executive and cou for all precincts, including estimated shares for
District 3. However I now have the problem that the true vote shares
per precinct may differ from those based on election day results,
since they don’t take into account the affects of early voting and
voting by mail (i.e., using absentee ballots).
...
`r format(rep_ce_en_votes_val, big.mark = ",")` votes compared to
`r format(dem_ce_en_votes_val, big.mark = ",")` for Calvin Ball.
However Ball did much better in early voting, and ended up winning
`r format(dem_ce_all_votes_val, big.mark = ",")` to
`r format(rep_ce_all_votes_val, big.mark = ",")`.
Put another way, Ball’s total vote was
`r round(100 * (dem_ce_all_vs_en_val - 1), 0)`% greater than his
election day vote total, while Allan Kittleman’s total vote was only
`r round(100 * (rep_ce_all_vs_en_val - 1), 0)`% greater than his
election day result.

This suggests a possible approach: estimate precinct-level overall
vote shares (including early voting and absentee ballots) by
multiplying each precinct’s election day results for Calvin Ball by
`r round(dem_ce_all_vs_en_val, 2)`,
and multiplying each precinct’s election day results for Allan
Kittleman by
`r round(rep_ce_all_vs_en_val, 2)`.

But, one might ask, since we have precinct-level numbers for turnout
by party, including early voting and absentee ballots by party, why
can’t we compute a multiplicative factor for each precinct
individually? The short answer is that the turnout statistics are for
registered voters, so such an approach would not account for
registered Democrats crossing party lines to vote Republican (or vice
versa), nor for independent voters. Using county-wide factors is thus
the preferred approach.

What about estimating per-precinct vote shares for county council
races?  We can look at the difference between election day votes and
all votes for the two parties’ county council candidates, again
ignoring District 3.  I’ve already calculated the vote totals, so I
just need to compute the ratio of all votes to election day votes for
each party.

```{r cc_ex3_all_vs_en}
dem_cc_ex3_all_vs_en_val = dem_cc_ex3_all_votes_val / dem_cc_ex3_en_votes_val

rep_cc_ex3_all_vs_en_val = rep_cc_ex3_all_votes_val / rep_cc_ex3_en_votes_val
```
...
`r round(100 * (dem_cc_ex3_all_vs_en_val - 1), 0)`% greater than their
combined vote on election day, while the combined total vote for all
Republican county council candidates outside District 3 was only
`r round(100 * (rep_cc_ex3_all_vs_en_val - 1), 0)`% greater than their
combined election day result.
...
est_all_votes_by_precinct_wide_tb <- est_en_votes_by_precinct_wide_tb %>%
  mutate(`County Executive-REP` = round(rep_ce_all_vs_en_val * `County Executive-REP`, 0),
         ...
         `County Council-REP` = round(rep_cc_ex3_all_vs_en_val * `County Council-REP`, 0),
         `County Council-DEM` = round(dem_cc_ex3_all_vs_en_val * `County Council-DEM`, 0))
```
...
`r dem_ce_all_votes_val` and
`r rep_ce_all_votes_val` respectively.
...
Democratic and Republican county executive candidates and county
council candidates (including a hypothetical Republican candidate in
District 3).
...
missing data for the 2018 general election county council race
in District 3, and the lack of precinct-level results for early
...
adjustment interacts with the adjustment the US Census Bureau applied
to census block data in an attempt to protect people’s privacy.
